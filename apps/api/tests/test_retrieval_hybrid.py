...
async def test_hybrid_retrieve(mock_db, mock_vector, mock_embed):
    ...
    chunks = [
        Chunk(
            ...
            document_id="d1",
            chunk_index=0,
            text="Approval required for external APIs.",
        ),
        Chunk(
            chunk_id="chunk-2",
            document_id="d1",
            chunk_index=1,
            text="Unrelated content about dragons.",
        ),
    ]
    ...
    service.settings.retrieval_mode = "hybrid"
    sources = await service.retrieve("approval external APIs", top_k=2)
